...
Call `this.insertBefore(new_node)` in either `enter` or `leave` to insert a new node before the current node.

Call `this.insertAfter(new_node)` in either `enter` or `leave` to insert a new node after the current node.
